...
DOCS_BUILDER_IMAGE_NAME='igl__libtrustbridge/docs_builder'


def _run_docs_builder(c,listen_port=False):
    run_options=['--rm','-it',f'-v "{ROOT_DIR}:/app"','--name igl__libtrustbridge__docs_builder']
    if listen_port:
        run_options.append('-p 8991:80')
