...
def _remove_required_message_params(data, index=-1, indexes=[]):
    ...
    return _remove_message_params(data, keys=[Message.required_attrs[i] for i in indexes])
...
def _generate_message_params():
    ...
    A = Country(msg_dict["sender"])
    B = Country(msg_dict["receiver"])
    subject = URI(msg_dict["subject"])
    obj = URI(msg_dict["obj"])
    predicate = URI(msg_dict["predicate"])
