...
class ElasticMQRepo:
    ...
    def _create_message_object(self, data):
        try:
            ...
            logger.error("Unable to deserealize data %s to Message object", data)
            raise

    def post(self, msg, delay_seconds=0):
        payload = json.dumps(msg, cls=MessageJSONEncoder)
        try:
    ...
    def post_job(self, payload, delay_seconds=0):
        ...
        self.client.send_message(
            QueueUrl=self.queue_url,
            MessageBody=json.dumps(payload, cls=MessageJSONEncoder),
            DelaySeconds=delay_seconds
        )
        return True
